...
void project::Tilemap::Load(const std::string& filename, int mapW, int mapH, int tileW, int tileH)
{
...
void project::Tilemap::AddLayer(const std::string& layer, TLayer tiles)
{
...
bool project::Tilemap::IsColliding(const std::string& layer, float x, float y, float w, float h, int* tileIndex)
{
        ...
        for (int y = 0; y < m_Height; y++)
        {
            for (int x = 0; x < m_Width; x++)
            {
                ...
                if (_idx > 0)
                {
                    ...
                    project::Engine::Get().Graphics().DrawTexture(
